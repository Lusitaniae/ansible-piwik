...
linked_containers=${linked_containers:-"--link redis:redis --link mysql:mysql"}
...
docker run --detach --volume="$PWD":/etc/ansible/roles/role_under_test:rw --name $container_id $linked_containers $opts geerlingguy/docker-$distro-ansible:latest $init
...
docker exec $container_id env TERM=xterm env ANSIBLE_FORCE_COLOR=1 ansible-playbook -vvv /etc/ansible/roles/role_under_test/tests/$playbook
